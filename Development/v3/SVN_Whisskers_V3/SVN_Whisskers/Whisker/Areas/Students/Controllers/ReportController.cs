...
namespace Whisker.Areas.Students.Controllers
{
    ...
    public class ReportController : Controller
    {
        ...
        public ActionResult Industry()
        {
            ViewBag.Active = "#li_Reports";
            GetAccountDetails();
            ViewBag.BatchCode = objUserInfo.BatchCode;
            ViewBag.BatchName = objUserInfo.BatchName;
            ViewBag.AccCode = objUserInfo.AccCode;
            ViewBag.AccName = objUserInfo.AccName;
            return View();
        }
        ...
        public JsonResult GetIndustryReportCampaignMatrix(string BatchCode)
        {
            ...
            object CampaignDetails = objDBentity.prcGetCampaign(BatchCode,objUserInfo.AccCode.ToString(), null).ToArray();
        ...
        public JsonResult GetIndustryReportRfaComparsnByAcc(string BatchCode)
        {
            ...
            object AccountDetails = objDBentity.RFA_ComparisonsByAcc(Convert.ToInt32(BatchCode), Convert.ToInt32(objUserInfo.AccCode)).ToArray();
            AllAccountDetail.Add(AccountDetails);
            var vg = Json(AllAccountDetail, JsonRequestBehavior.AllowGet);
            return Json(AllAccountDetail, JsonRequestBehavior.AllowGet);
        }
        public JsonResult GetIndustryReportRfaComparsnByRfa(string BatchCode)
        {
            ...
            object AccountDetails = objDBentity.RFA_ComparisonsByRFA(Convert.ToInt32(BatchCode), Convert.ToInt32(objUserInfo.AccCode)).ToArray();
            AllAccountDetail.Add(AccountDetails);
            var vg = Json(AllAccountDetail, JsonRequestBehavior.AllowGet);
            return Json(AllAccountDetail, JsonRequestBehavior.AllowGet);
        }
        ...
        public ActionResult GetIndustryReportCampaignMatrixExcel(string id)
        {
            ...
            //Bind data list from edmx
            objUserInfo = (UserLoginInfo)Session["LoginInfo"];
            var getCampaign = objDBentity.prcGetCampaign(BatchCode, objUserInfo.AccCode.ToString(), null).ToList();
            if (getCampaign != null && getCampaign.Any())
            {
                var HeaderCSS = ".download-tbl{width: 100 %; border: 0px;}" +
                    ".download-tbl th{background:#e4e4e4;color:#000; padding: 6px 12px;text-align:center; border:0px;}" +
        ...
        public ActionResult GetIndustryReportRfaComparsnByAccExcel(string id)
        {
            ...
            //Bind data list from edmx
            objUserInfo = (UserLoginInfo)Session["LoginInfo"];
            var getCampaign = objDBentity.RFA_ComparisonsByAcc(Convert.ToInt32(BatchCode), Convert.ToInt32(objUserInfo.AccCode)).ToList();
            if (getCampaign != null && getCampaign.Any())
            {
                var HeaderCSS = ".download-tbl{width: 100 %; border: 0px;}" +
                    ".download-tbl th{background:#e4e4e4;color:#000; padding: 6px 12px;text-align:center; border:0px;}" +
