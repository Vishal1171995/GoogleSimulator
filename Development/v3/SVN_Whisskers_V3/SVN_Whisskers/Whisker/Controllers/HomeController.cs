...
namespace Whisker.Controllers
{
    ...
    public class HomeController : Controller
    {
                ...
                if (userDetails.UserType == 0)
                {
                    ...
                    else
                    {
                        ...
                        if(CompareDateResult <= 0)
                            ...
                            return RedirectToAction("BatchSwitcher", "Home", new { area = "Students" });
                        else
        ...
        public ActionResult Register(FormCollection fc)
        {
            ...
            UserInfo.FirstName = fc["fname"];
            UserInfo.LastName = fc["lname"];
            UserInfo.MobileNo = fc["Contact"];
            UserInfo.EMAILID = fc["RegEmail"];
            if (fc["regPassword"] == fc["regCPassword"])
            {
                UserInfo.Password = fc["regPassword"];
            }

            UserInfo.Organization = fc["Company"];

            UserInfo.Location = "LC00009";
            //LC00009
            objDBentity.prcCreateStudent(UserInfo.FirstName, UserInfo.MiddleName, UserInfo.LastName, UserInfo.Location, UserInfo.EMAILID, UserInfo.Password, UserInfo.MobileNo, "LC00009", null, "INSERT",true);
           // cmnF.SendPasscodeToMail(stdtDtl.Email, stdtDtl.FirstName, randomPwd, stdtDtl.Email, "whisskers login details");
            ...
            var userDetails = objDBentity.UserMasters.Where(x => x.Email == UserInfo.EMAILID && x.Password == UserInfo.Password && x.IsActive == true).FirstOrDefault();
            if (userDetails != null)
            {
                ...
                ViewBag.RegistrationFail = "Did not Login";
        ...
        public JsonResult GetBatch()
        {
           ...
           // objUserInfo = (UserLoginInfo)Session["LoginInfo"];
            var userBatch = objDBentity.prcGetBatches().ToList();
            return Json(userBatch, JsonRequestBehavior.AllowGet);
        }
    }
}
